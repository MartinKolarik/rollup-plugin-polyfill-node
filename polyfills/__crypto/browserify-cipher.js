...
inherits$a(CipherBase$1, Transform$7);
...
Cipher$4.prototype._init = function _init() {
  // Might be overrided
};

Cipher$4.prototype.update = function update(data) {
  if (data.length === 0)
    return [];
...
Cipher$4.prototype._buffer = function _buffer(data, off) {
...
Cipher$4.prototype._flushBuffer = function _flushBuffer(out, off) {
  this._update(this.buffer, 0, out, off);
  this.bufferOff = 0;
  return this.blockSize;
};

Cipher$4.prototype._updateEncrypt = function _updateEncrypt(data) {
  var inputOff = 0;
  var outputOff = 0;
...
Cipher$4.prototype._updateDecrypt = function _updateDecrypt(data) {
  var inputOff = 0;
  var outputOff = 0;
...
Cipher$4.prototype.final = function final(buffer) {
  var first;
  if (buffer)
...
Cipher$4.prototype._pad = function _pad(buffer, off) {
  if (off === 0)
    return false;
...
Cipher$4.prototype._finalEncrypt = function _finalEncrypt() {
...
Cipher$4.prototype._unpad = function _unpad(buffer) {
  return buffer;
};

Cipher$4.prototype._finalDecrypt = function _finalDecrypt() {
...
function DES$1 (opts) {
  ...
  var key = opts.key;
  if (!Buffer$e.isBuffer(key)) {
    key = Buffer$e.from(key);
  }
  if (modeName === 'des-ede' || modeName === 'des-ede-cbc') {
    key = Buffer$e.concat([key, key.slice(0, 8)]);
  }
  var iv = opts.iv;
  if (!Buffer$e.isBuffer(iv)) {
    iv = Buffer$e.from(iv);
  }
...
DES$1.prototype._update = function (data) {
  return Buffer$e.from(this._des.update(data))
};
DES$1.prototype._final = function () {
  return Buffer$e.from(this._des.final())
};
...
function encryptStart (self, data, decrypt) {
  ...
  self._prev = Buffer$d.concat([self._prev, decrypt ? data : out]);
  return out
}

cfb.encrypt = function (self, data, decrypt) {
  var out = Buffer$d.allocUnsafe(0);
  var len;

  while (data.length) {
    if (self._cache.length === 0) {
      self._cache = self._cipher.encryptBlock(self._prev);
      self._prev = Buffer$d.allocUnsafe(0);
    }

    if (self._cache.length <= data.length) {
      len = self._cache.length;
      out = Buffer$d.concat([out, encryptStart(self, data.slice(0, len), decrypt)]);
      data = data.slice(len);
    } else {
      out = Buffer$d.concat([out, encryptStart(self, data, decrypt)]);
      break
    }
  }
...
function encryptByte$1 (self, byteParam, decrypt) {
  ...
  self._prev = Buffer$c.concat([
    self._prev.slice(1),
    Buffer$c.from([decrypt ? byteParam : out])
  ]);
...
function shiftIn (buffer, value) {
  ...
  var i = -1;
  var out = Buffer$b.allocUnsafe(buffer.length);
  buffer = Buffer$b.concat([buffer, Buffer$b.from([value])]);
...
cfb1.encrypt = function (self, chunk, decrypt) {
  var len = chunk.length;
  var out = Buffer$b.allocUnsafe(len);
  var i = -1;
...
ctr.encrypt = function (self, chunk) {
  ...
  self._cache = Buffer$a.concat([
    self._cache,
    Buffer$a.allocUnsafe(chunkNum * blockSize)
  ]);
...
function asUInt32Array (buf) {
  if (!Buffer$9.isBuffer(buf)) buf = Buffer$9.from(buf);
...
AES.prototype.encryptBlock = function (M) {
  ...
  var buf = Buffer$9.allocUnsafe(16);
...
var ZEROES = Buffer$8.alloc(16, 0);
...
GHASH$1.prototype.final = function (abl, bl) {
  if (this.cache.length) {
    this.ghash(Buffer$8.concat([this.cache, ZEROES], 16));
  }
...
function calcIv (self, iv, ck) {
  if (iv.length === 12) {
    self._finID = Buffer$7.concat([iv, Buffer$7.from([0, 0, 0, 1])]);
    return Buffer$7.concat([iv, Buffer$7.from([0, 0, 0, 2])])
  }
  ...
  if (toPad) {
    toPad = 16 - toPad;
    ghash.update(Buffer$7.alloc(toPad, 0));
  }
  ghash.update(Buffer$7.alloc(8, 0));
  var ivBits = len * 8;
  var tail = Buffer$7.alloc(8);
  tail.writeUIntBE(ivBits, 0, 8);
  ghash.update(tail);
  self._finID = ghash.state;
  var out = Buffer$7.from(self._finID);
  incr32(out);
  return out
}
function StreamCipher$3 (mode, key, iv, decrypt) {
  ...
  this._prev = Buffer$7.from(iv);
  this._cache = Buffer$7.allocUnsafe(0);
  this._secCache = Buffer$7.allocUnsafe(0);
...
StreamCipher$3.prototype.getAuthTag = function getAuthTag () {
  if (this._decrypt || !Buffer$7.isBuffer(this._authTag)) throw new Error('Attempting to get auth tag in unsupported state')
...
inherits_browserExports(Transform$4, Duplex);
...
inherits_browserExports(PassThrough, Transform$3);
...
function EVP_BytesToKey (password, salt, keyBits, ivLen) {
  if (!Buffer$3.isBuffer(password)) password = Buffer$3.from(password, 'binary');
  if (salt) {
    if (!Buffer$3.isBuffer(salt)) salt = Buffer$3.from(salt, 'binary');
    if (salt.length !== 8) throw new RangeError('salt should be Buffer with 8 byte length')
  }

  var keyLen = keyBits / 8;
  var key = Buffer$3.alloc(keyLen);
  var iv = Buffer$3.alloc(ivLen || 0);
  var tmp = Buffer$3.alloc(0);
...
browser$1.createCipher = browser$1.Cipher = ciphers.createCipher;
browser$1.createCipheriv = browser$1.Cipheriv = ciphers.createCipheriv;
browser$1.createDecipher = browser$1.Decipher = deciphers.createDecipher;
browser$1.createDecipheriv = browser$1.Decipheriv = deciphers.createDecipheriv;
browser$1.listCiphers = browser$1.getCiphers = getCiphers$1;
...
var createCipher_1 = browser$2.createCipher = Cipher = browser$2.Cipher = createCipher;
var createCipheriv_1 = browser$2.createCipheriv = Cipheriv = browser$2.Cipheriv = createCipheriv;
var createDecipher_1 = browser$2.createDecipher = Decipher = browser$2.Decipher = createDecipher;
var createDecipheriv_1 = browser$2.createDecipheriv = Decipheriv = browser$2.Decipheriv = createDecipheriv;
var listCiphers = browser$2.listCiphers = getCiphers_1 = browser$2.getCiphers = getCiphers;

export { Cipher, Cipheriv, Decipher, Decipheriv, createCipher_1 as createCipher, createCipheriv_1 as createCipheriv, createDecipher_1 as createDecipher, createDecipheriv_1 as createDecipheriv, browser$2 as default, getCiphers_1 as getCiphers, listCiphers };
