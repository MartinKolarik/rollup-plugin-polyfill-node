...
inherits$r(HashBase$2, Transform$5);
...
var Buffer$z = require$$0$1.Buffer;
...
inherits$p(RIPEMD160$4, HashBase);
...
RIPEMD160$4.prototype._digest = function () {
  ...
  // produce result
  var buffer = Buffer$z.alloc ? Buffer$z.alloc(20) : new Buffer$z(20);
  buffer.writeInt32LE(this._a, 0);
  buffer.writeInt32LE(this._b, 4);
  buffer.writeInt32LE(this._c, 8);
  buffer.writeInt32LE(this._d, 12);
...
function Hash$7 (blockSize, finalSize) {
  this._block = Buffer$y.alloc(blockSize);
...
inherits$i(CipherBase, Transform$4);
...
function Hmac$3 (alg, key) {
  ...
  if (key.length > blocksize) {
    ...
    key = Buffer$q.concat([key, ZEROS$2], blocksize);
  }
...
function Hmac$2 (alg, key) {
  ...
  if (key.length > blocksize) {
    ...
    key = Buffer$p.concat([key, ZEROS$1], blocksize);
  }
...
function randomBytes$1 (size, cb) {
  ...
  var bytes = Buffer$o.allocUnsafe(size);
...
function EC$2(options) {
  ...
  if (typeof options === 'string') {
    assert$4(Object.prototype.hasOwnProperty.call(curves$3, options),
...
var buffer$1 = require$$0$1;
var Buffer$n = buffer$1.Buffer;
...
for (key$1 in Buffer$n) {
  if (!Buffer$n.hasOwnProperty(key$1)) continue
  if (key$1 === 'allocUnsafe' || key$1 === 'allocUnsafeSlow') continue
  Safer[key$1] = Buffer$n[key$1];
}

safer.Buffer.prototype = Buffer$n.prototype;
...
if (!Safer.from || Safer.from === Uint8Array.from) {
  Safer.from = function (value, encodingOrOffset, length) {
    ...
    return Buffer$n(value, encodingOrOffset, length)
  };
}
...
const inherits$a = inherits_browserExports;
...
inherits$a(ReporterError, Error);
...
const inherits$9 = inherits_browserExports;
...
inherits$9(DecoderBuffer$2, Reporter$1);
...
function EncoderBuffer$1(value, reporter) {
  if (Array.isArray(value)) {
    ...
    this.value = value;
    this.length = Buffer$m.byteLength(value);
  } else if (Buffer$m.isBuffer(value)) {
    this.value = value;
...
EncoderBuffer$1.prototype.join = function join(out, offset) {
  if (!out)
    out = Buffer$m.alloc(this.length);
  if (!offset)
    offset = 0;
  ...
  if (Array.isArray(this.value)) {
    ...
    else if (Buffer$m.isBuffer(this.value))
...
const inherits$8 = inherits_browserExports;
...
inherits$7(PEMEncoder, DEREncoder);
...
const inherits$6 = inherits_browserExports;
...
inherits$5(PEMDecoder, DERDecoder);
var pem = PEMDecoder;
...
PEMDecoder.prototype.decode = function decode(data, options) {
  ...
  const input = Buffer$k.from(base64, 'base64');
...
var toBuffer$2 = function (thing, encoding, name) {
  if (Buffer$7.isBuffer(thing)) {
    return thing
  } else if (typeof thing === 'string') {
    return Buffer$7.from(thing, encoding)
  } else if (ArrayBuffer.isView(thing)) {
    return Buffer$7.from(thing.buffer)
  } else {
...
Sign.prototype.update = function update (data, enc) {
  if (typeof data === 'string') data = Buffer$1.from(data, enc);
...
Verify.prototype.verify = function verifyMethod (key, sig, enc) {
  if (typeof sig === 'string') sig = Buffer$1.from(sig, enc);
