...
for (key$1 in Buffer$A) {
  if (!Buffer$A.hasOwnProperty(key$1)) continue
  if (key$1 === 'allocUnsafe' || key$1 === 'allocUnsafeSlow') continue
  Safer[key$1] = Buffer$A[key$1];
}
...
const inherits$k = inherits_browserExports;
...
inherits$k(ReporterError, Error);
...
inherits$j(DecoderBuffer$2, Reporter$1);
...
function EncoderBuffer$1(value, reporter) {
  if (Array.isArray(value)) {
    ...
    this.value = value;
    this.length = Buffer$z.byteLength(value);
  } else if (Buffer$z.isBuffer(value)) {
    this.value = value;
...
EncoderBuffer$1.prototype.join = function join(out, offset) {
  if (!out)
    out = Buffer$z.alloc(this.length);
  if (!offset)
    offset = 0;
  ...
  if (Array.isArray(this.value)) {
    ...
    else if (Buffer$z.isBuffer(this.value))
...
const inherits$i = inherits_browserExports;
const Buffer$y = safer_1.Buffer;
const Node$1 = node;
...
DERNode$1.prototype._encodeComposite = function encodeComposite(tag,
  ...
  const header = Buffer$y.alloc(1 + 1 + lenOctets);
  header[0] = encodedTag;
...
DERNode$1.prototype._encodeStr = function encodeStr(str, tag) {
  if (tag === 'bitstr') {
    ...
    const buf = Buffer$y.alloc(str.length * 2);
...
DERNode$1.prototype._encodeObjid = function encodeObjid(id, values, relative) {
  ...
  const objid = Buffer$y.alloc(size);
...
DERNode$1.prototype._encodeInt = function encodeInt(num, values) {
  ...
  if (typeof num !== 'number' && !Buffer$y.isBuffer(num)) {
    const numArray = num.toArray();
    if (!num.sign && numArray[0] & 0x80) {
      numArray.unshift(0);
    }
    num = Buffer$y.from(numArray);
  }

  if (Buffer$y.isBuffer(num)) {
    let size = num.length;
    if (num.length === 0)
      size++;

    const out = Buffer$y.alloc(size);
    num.copy(out);
    if (num.length === 0)
      out[0] = 0;
  ...
  return this._createEncoderBuffer(Buffer$y.from(out));
};
...
const inherits$h = inherits_browserExports;
...
inherits$h(PEMEncoder, DEREncoder);
...
const inherits$g = inherits_browserExports;
...
const inherits$f = inherits_browserExports;
const Buffer$x = safer_1.Buffer;
...
inherits$f(PEMDecoder, DERDecoder);
var pem = PEMDecoder;
...
PEMDecoder.prototype.decode = function decode(data, options) {
  ...
  const input = Buffer$x.from(base64, 'base64');
...
function throwIfNotStringOrBuffer (val, prefix) {
  if (!Buffer$w.isBuffer(val) && typeof val !== 'string') {
...
function HashBase$2 (blockSize) {
  ...
  this._block = Buffer$w.allocUnsafe(blockSize);
...
HashBase$2.prototype.update = function (data, encoding) {
  ...
  if (!Buffer$w.isBuffer(data)) data = Buffer$w.from(data, encoding);
...
MD5$3.prototype._digest = function () {
  ...
  // produce result
  var buffer = Buffer$v.allocUnsafe(16);
...
function EVP_BytesToKey (password, salt, keyBits, ivLen) {
  if (!Buffer$u.isBuffer(password)) password = Buffer$u.from(password, 'binary');
  if (salt) {
    if (!Buffer$u.isBuffer(salt)) salt = Buffer$u.from(salt, 'binary');
    if (salt.length !== 8) throw new RangeError('salt should be Buffer with 8 byte length')
  }

  var keyLen = keyBits / 8;
  var key = Buffer$u.alloc(keyLen);
  var iv = Buffer$u.alloc(ivLen || 0);
  var tmp = Buffer$u.alloc(0);
...
function encryptStart (self, data, decrypt) {
  ...
  self._prev = Buffer$t.concat([self._prev, decrypt ? data : out]);
  return out
}

cfb.encrypt = function (self, data, decrypt) {
  var out = Buffer$t.allocUnsafe(0);
  var len;

  while (data.length) {
    if (self._cache.length === 0) {
      self._cache = self._cipher.encryptBlock(self._prev);
      self._prev = Buffer$t.allocUnsafe(0);
    }

    if (self._cache.length <= data.length) {
      len = self._cache.length;
      out = Buffer$t.concat([out, encryptStart(self, data.slice(0, len), decrypt)]);
      data = data.slice(len);
    } else {
      out = Buffer$t.concat([out, encryptStart(self, data, decrypt)]);
      break
    }
  }
...
function encryptByte$1 (self, byteParam, decrypt) {
  ...
  self._prev = Buffer$s.concat([
    self._prev.slice(1),
    Buffer$s.from([decrypt ? byteParam : out])
  ]);
...
function shiftIn (buffer, value) {
  ...
  var i = -1;
  var out = Buffer$r.allocUnsafe(buffer.length);
  buffer = Buffer$r.concat([buffer, Buffer$r.from([value])]);
...
ctr.encrypt = function (self, chunk) {
  ...
  self._cache = Buffer$q.concat([
    self._cache,
    Buffer$q.allocUnsafe(chunkNum * blockSize)
  ]);
...
function asUInt32Array (buf) {
  if (!Buffer$p.isBuffer(buf)) buf = Buffer$p.from(buf);
...
AES.prototype.encryptBlock = function (M) {
  ...
  var buf = Buffer$p.allocUnsafe(16);
...
var PADDING = Buffer$k.alloc(16, 0x10);
...
Splitter$1.prototype.flush = function () {
  ...
  var padBuff = Buffer$k.allocUnsafe(len);
  ...
  return Buffer$k.concat([this.cache, padBuff])
};
...
var fixProc$1 = function (okey, password) {
  ...
  if (!match) {
    ...
    decrypted = Buffer$i.from(match2[2].replace(/[\r\n]/g, ''), 'base64');
  } else {
    var suite = 'aes' + match[1];
    var iv = Buffer$i.from(match[2], 'hex');
    var cipherText = Buffer$i.from(match[3].replace(/[\r\n]/g, ''), 'base64');
    var cipherKey = evp(password, iv.slice(0, 8), parseInt(match[1], 10)).key;
    var out = [];
    var cipher = ciphers$1.createDecipheriv(suite, cipherKey, iv);
    out.push(cipher.update(cipherText));
    out.push(cipher.final());
    decrypted = Buffer$i.concat(out);
  }
...
RIPEMD160$2.prototype._digest = function () {
  ...
  // produce result
  var buffer = Buffer$h.alloc ? Buffer$h.alloc(20) : new Buffer$h(20);
  buffer.writeInt32LE(this._a, 0);
  buffer.writeInt32LE(this._b, 4);
  buffer.writeInt32LE(this._c, 8);
  buffer.writeInt32LE(this._d, 12);
...
function Hash$7 (blockSize, finalSize) {
  this._block = Buffer$g.alloc(blockSize);
...
function Hmac (alg, key, saltLen) {
  ...
  if (key.length > blocksize) {
    ...
    key = Buffer$8.concat([key, ZEROS], blocksize);
  }

  var ipad = Buffer$8.allocUnsafe(blocksize + sizes[alg]);
  var opad = Buffer$8.allocUnsafe(blocksize + sizes[alg]);
  for (var i = 0; i < blocksize; i++) {
    ipad[i] = key[i] ^ 0x36;
    opad[i] = key[i] ^ 0x5C;
  }

  var ipad1 = Buffer$8.allocUnsafe(blocksize + saltLen + 4);
...
function randomBytes$2 (size, cb) {
  ...
  var bytes = Buffer$5.allocUnsafe(size);
